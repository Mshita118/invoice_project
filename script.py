...
from invoice_database import insert_client, fetch_clients, insert_invoice, fetch_invoices, insert_detail, fetch_details, insert_log, fetch_logs
...
def open_client_form():
    ...
    tk.Label(form, text="連絡先：").grid(row=1, column=0, padx=10, pady=10)
    ...
    def save_invoice():
        ...
        if client_id and date and subtotal and total:
            try:
                ...
                insert_log(f"請求情報作成: クライアントID {client_id}, 日付 {date}")  # ログを記録
    ...
    def save_details():
        ...
        if client_id and building_count and unit_price:
            try:
                ...
                insert_log(f"明細作成: クライアントID {client_id}, 棟数 {building_count}, 単価 {unit_price}")  # ログを記録
...
def show_logs():
    logs = fetch_logs()  # ログを取得
    log_window = tk.Toplevel()
    log_window.title("ログ一覧")
    log_window.geometry("400x300")

    log_list = tk.Listbox(log_window, width=50, height=20)
    log_list.pack(padx=10, pady=10)

    for log in logs:
        log_list.insert(tk.END, f"[{log[2]}] {log[1]}")
...
log_button = tk.Button(root, text="ログ表示", command=show_logs)
